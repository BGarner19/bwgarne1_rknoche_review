...
public class RandomNumber {
         ...
         */
        public RandomNumber() {
                currentSeed = System.currentTimeMillis() % _MODULUS;
        }
        ...
        public RandomNumber(long seed) {
                currentSeed = Math.abs(seed) % _MODULUS;
        }
        ...
        public double nextDoubleRand() {
                long temp = _MULTIPLIER*(currentSeed%_QUOT) - 
                                _REM*(currentSeed/_QUOT);
                currentSeed = (temp > 0) ? temp : temp + _MODULUS;
                return (double) currentSeed / (double) _MODULUS;
        }
